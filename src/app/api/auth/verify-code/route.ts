...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!email || !code) {
      return jsonError("E-mail et code requis", 400);
    }
    ...
    if (!loginCode) {
      return jsonError("Aucun code actif trouvé", 400);
    }
    if (loginCode.expiresAt < new Date()) {
      return jsonError("Le code a expiré", 400);
    }
    ...
    }); /* Supression de toues les sessions antérieures (toute ip) */
    ...
    return jsonError(`Erreur lors de la vérification : ${error}`, 500);
  }
}
