...
import { browneVsEquity, VERDICT_HEX, VERDICT_ORDER, type BrowneVerdict } from "./helpers";

interface Point {
  iso: string;
  name: string;
  x: number; // réduction drawdown (pts) — droite = meilleure protection
  y: number; // écart de rendement (pts) — haut = Browne fait mieux
  verdict: BrowneVerdict;
  color: string;
  brReal: number | null;
  actReal: number | null;
  ecartVol: number | null;
  volBrowne: number | null;
  volActions: number | null;
  ddBrowne: number | null;
  ddActions: number | null;
  sharpeBrowne: number | null;
...
export function BrowneVsEquityMatrix({
  ...
  const points = useMemo<Point[]>(() => {
    ...
    for (const r of rows) {
      ...
      out.push({
        ...
        brReal: r.real?.annualized ?? null,
        actReal: r.equityReal?.annualized ?? null,
        ecartVol: v.ecartVol,
        volBrowne: r.real?.volatility ?? null,
        volActions: r.equityReal?.volatility ?? null,
        ddBrowne: r.real?.maxDrawdown ?? null,
        ddActions: r.equityReal?.maxDrawdown ?? null,
        sharpeBrowne: r.real?.sharpe ?? null,
  ...
  // Domaines : on inclut toujours 0 sur les deux axes → les lignes de référence
  // (protection nulle / rendement égal) et les 4 quadrants restent visibles.
    ...
    <div>
      <div className="relative w-full" style={{ height: HEIGHT }}>
        ...
        <div className="absolute border-b-[1.5px] border-l-[1.5px] border-foreground/20" style={{ left: PAD.left, right: PAD.right, top: PAD.top, bottom: PAD.bottom }}>
          {/* Halo « meilleur » : haut-droite (Browne fait mieux + protège mieux) */}
