...
// Classes Tailwind LITTÉRALES (aplat carte + pastille) par profil.
const MAP_CLASS: Record<QuadrantsVerdict, { fill: string; dot: string }> = {
...
export function QuadrantsVerdictMap({ rows, region }: { rows: QuadrantModelRow[]; region: Region }) {
  const veByCode = useMemo(() => new Map(rows.map((r) => [r.countryCode, quadrantsVsEquity(r)])), [rows]);
  ...
  const items = useMemo<GeoItem[]>(
    () =>
      rows.flatMap((r) => {
        ...
        if (!v) return [];
        return [{ code: r.countryCode, name: r.countryFr ?? r.countryCode, fillClass: MAP_CLASS[v].fill, dotClass: MAP_CLASS[v].dot }];
      }),
    [rows, veByCode],
  );
  ...
  return (
    <GeoChoropleth
      items={items}
      region={region}
      title="Carte des profils 4 Quadrants vs Actions"
          ...
          >
            ...
            {ve?.verdict ? (
              <div className="mt-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-xs">
                ...
                <span className="text-right tabular-nums">{pts(ve.ecartReturn)}</span>
                <span className="text-muted-foreground">Réduction drawdown :</span>
                <span className="text-right tabular-nums">{pts(ve.drawdownReduction)}</span>
                <span className="text-muted-foreground">Écart volatilité :</span>
                <span className="text-right tabular-nums">{pts(ve.ecartVol)}</span>
              </div>
            ) : (
      ...
      renderBelow={(framedCodes) => {
        const counts = {} as Record<QuadrantsVerdict, number>;
