...
import { useState, useRef, useTransition, useCallback, useSyncExternalStore, useEffect } from "react";
import { AlertCircleIcon, CheckCircle2Icon, CircleDashedIcon, LoaderCircleIcon, XIcon } from "lucide-react";
...
export const SERIE_COLORS: Record<"s1" | "s2", SeriesColor> = {
  s1: {
    hex          : "#502239",
    badgeClassName: "bg-[rgba(80,34,57,0.10)] border-[rgba(80,34,57,0.25)] text-[#502239] dark:bg-[rgba(80,34,57,0.20)] dark:border-[rgba(80,34,57,0.45)] dark:text-[#CE8DAD]",
  },
  s2: {
    hex          : "#2E4A6B",
    badgeClassName: "bg-[rgba(46,74,107,0.10)] border-[rgba(46,74,107,0.25)] text-[#2E4A6B] dark:bg-[rgba(46,74,107,0.20)] dark:border-[rgba(46,74,107,0.45)] dark:text-[#94B0D1]",
  },
};
...
function SeriesFilterRow({ options, hierarchy, label, color, error, initialSelection, onChange }: RowProps) {
  ...
  const [transform, setTransform] = useState<SeriesTransform>     (initialSelection?.transform ?? "defaut");
  ...
  function handleTypeChange(i: SelectItem) {
    ...
    onChange(country && classe ? { country, classe, type: newType, sourceCcy: newCcy, transform } : null);
  }
  ...
  return (
    <div className="rounded-lg border bg-muted/50 p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        {color
          ? <span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-semibold", color.badgeClassName)}>{label}</span>
          : <p className="text-sm font-semibold">{label}</p>
        }
        {isReady
          ? <span className="inline-flex items-center gap-1.5 rounded-md border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-600"><CheckCircle2Icon className="size-3.5" />Prête</span>
          : <span className="inline-flex items-center gap-1.5 rounded-md border border-border bg-muted/50 px-2 py-0.5 text-xs font-medium text-muted-foreground"><CircleDashedIcon className="size-3.5" />Incomplète</span>
        }
      </div>
      ...
      <div className="flex gap-3">
        ...
        />
        <SelectDropdown items={TRANSFORM_ITEMS} value={transform} onChange={handleTransformChange} />
      </div>
...
export function PlaygroundClient({ options, hierarchy }: Props) {
  ...
  const [displayCcy, setDisplayCcy] = useLsState<string | undefined>   ("playground.displayCcy", undefined);
  const [persistedDr, setPersistedDr] = useLsState<PersistedDateRange> ("playground.dateRange",  null);
  ...
  const fetchSeries = useCallback(async (sel: SeriesSelection) => {
    ...
    if (cache.current.has(key)) return { data: cache.current.get(key)!, error: null as string | null };
    ...
    <div className="flex flex-col gap-4 w-fit mx-auto">
      ...
      <div className="flex flex-col gap-4">
        <SeriesFilterRow
          ...
          options={options} hierarchy={hierarchy} label="Série 1" color={SERIE_COLORS.s1} error={error1}
          initialSelection={series1}
          onChange={sel => { setSeries1(sel); setLoaded1(null); setError1(null); }}
        />
        <SeriesFilterRow
          key={series2 ? `s2-${cacheKey(series2)}` : "s2-empty"}
          options={options} hierarchy={hierarchy} label="Série 2" color={SERIE_COLORS.s2} error={error2}
          initialSelection={series2}
          onChange={sel => { setSeries2(sel); setLoaded2(null); setError2(null); }}
        />
      </div>
      ...
      {isPending && (
        <div className="rounded-lg border bg-muted/50 p-6 flex items-center justify-center gap-2 text-sm text-muted-foreground">
          <LoaderCircleIcon className="size-4 animate-spin" />Chargement des données…
        </div>
      )}
