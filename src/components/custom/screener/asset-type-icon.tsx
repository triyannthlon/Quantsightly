...
export type AssetKind = "stock" | "etf" | "index" | "crypto" | "forex" | "bond";
...
export function assetKind(exchangeCode?: string | null, typeRaw?: string | null): AssetKind {
  if (exchangeCode === "GBOND" || typeRaw === "BOND") return "bond";
...
/** Vrai uniquement pour les types qui ont un pays (stock / ETF / bond). */
export function hasCountry(kind: AssetKind): boolean {
  return kind === "stock" || kind === "etf" || kind === "bond";
}

const MAP: Record<AssetKind, { Icon: LucideIcon; cls: string }> = {
  stock: {
    Icon: Building2,
    cls: "bg-blue-100   text-blue-600   dark:bg-blue-500/15   dark:text-blue-400",
  },
  etf: {
    Icon: Layers,
    cls: "bg-violet-100 text-violet-600 dark:bg-violet-500/15 dark:text-violet-400",
  },
  index: {
  ...
  forex: {
    Icon: Banknote,
    cls: "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400",
  },
  bond: {
    Icon: Landmark,
