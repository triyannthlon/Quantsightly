...
import { KPI_TOOLTIPS } from "./kpi-tooltips";
...
function SharpeGauge({ value }: { value: number }) {
  ...
  const color = value < 0 ? "#f04438" : value < 1 ? "#fb923c" : value < 2 ? "#34d399" : "#10b981";
...
const VOLATILITY_GRADIENT =
  "linear-gradient(to right, #10b981 0%, #f59e0b 30%, #fb923c 50%, #ef4444 80%)";
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative mt-1 h-3">
        ...
        <span
          className="absolute text-[9px] text-muted-foreground/60 leading-none -translate-x-1/2"
          style={{ left: "30%" }}
        >
          15
        </span>
...
export function MetricCard({
  ...
  const hasGauge =
    !loading && !hasSparkline && (gaugeValue !== undefined || volatilityValue !== undefined);
  ...
  return (
    ...
    >
      ...
      {tooltip ? (
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className="absolute top-3 right-3 cursor-help outline-none focus-visible:ring-2 focus-visible:ring-ring rounded"
              >
                <Info className="h-[14px] w-[14px] text-muted-foreground/40 hover:text-muted-foreground/80 transition-colors" />
              </button>
            </TooltipTrigger>
        ...
        <Info className="absolute top-3 right-3 h-[14px] w-[14px] text-muted-foreground/40 cursor-default" />
      )}
        ...
        <div className="mt-3">
          {loading ? (
            ...
            <>
              ...
              {context && (
                <span className="text-xs text-muted-foreground mt-1 block">{context}</span>
              )}
              {qualLabel && (
                <span
                  className={cn(
