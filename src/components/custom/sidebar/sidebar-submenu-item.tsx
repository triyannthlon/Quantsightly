...
import { sidebarItemVariants, sidebarSubItemVariants } from "@/lib/navigation/sidebar/sidebar-variants";
...
/********************** SidebarSubmenuItem *****/
export default function SidebarSubmenuItem({item, pathname, density = "normal",}: { item: NavItem; pathname: string; density?: "compact" | "normal"; })
                        {//SidebarSubmenuItem
                        ...
                        const parentClassName = cn(sidebarItemVariants({
                                                                       variant  : getVariant(anyChildActive, item.tone),
                                                                       collapsed: !showLabels                          ,
                                                                       density,
                                                                       }),"menu-hover-gradient", "menu-active-indicator");
                                                                           ...
                                                                           return (
                                                                                  <DropdownMenuItem key={child.href} asChild>
                                                                                   <Link      href={child.href}
                                                                                         className={cn("flex w-full items-center rounded-sm px-2 py-2 text-sm outline-none", childActive && "font-medium")}>
                                                                                    {child.label}
                                                                                   </Link>
                                                                                  </DropdownMenuItem>
                                                                                  );
                               ...
                               <Collapsible open={isOpenInline} onOpenChange={() => toggleSubmenu(item.key)}>
                                <CollapsibleTrigger asChild>
                                 <button      type="button"
                                  ...
                                  <ChevronDown className={cn(               "ml-auto h-5 w-5 transition-transform duration-500 ease-[cubic-bezier(.2,.8,.2,1)]",
                                                             isOpenInline ? "rotate-180 text-brand-500 dark:text-brand-400"
                                ...
                                <div className={cn("submenu-inner", density === "compact" ? "pt-2" : "pt-3")}>
                                 <div className={cn("submenu-fade menu-submenu",density === "compact" ? "space-y-1.5" : "space-y-2.0")}>
                                 ...
                                 {item.children?.map((child) =>
                                                 ...
                                                 const childClassName = cn(sidebarSubItemVariants({variant: getVariant(childActive, child.tone), density,}), "menu-subitem-dot","menu-active-indicator","menu-hover-gradient",childActive && "menu-subitem-dot-active");
