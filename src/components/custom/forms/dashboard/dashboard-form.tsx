...
import { AssetPanel, type Period } from "@/components/custom/asset-panel/asset-panel";
import { usePeriod } from "@/hooks/period/period-context";
import {
  useDashboardFavorites,
  type DashboardFavorite,
} from "@/hooks/watchlist/use-dashboard-favorites";
    ...
    <div className="flex flex-col items-center justify-center py-24 text-center gap-5">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        <Button variant="outline" size="sm" asChild>
          <Link href="/screener/asset-stock">
            <BarChart3 className="h-3.5 w-3.5" />
            Actions
          </Link>
        </Button>
        <Button variant="outline" size="sm" asChild>
          <Link href="/screener/asset-etf">
            <BarChart3 className="h-3.5 w-3.5" />
            ETF
          </Link>
        </Button>
        <Button variant="outline" size="sm" asChild>
          <Link href="/screener/asset-crypto">
            <BarChart3 className="h-3.5 w-3.5" />
            Crypto
          </Link>
        </Button>
        <Button variant="outline" size="sm" asChild>
          <Link href="/screener/asset-index">
            <BarChart3 className="h-3.5 w-3.5" />
            Indices
          </Link>
        </Button>
        <Button variant="outline" size="sm" asChild>
          <Link href="/screener/asset-currency">
            <BarChart3 className="h-3.5 w-3.5" />
            Devises
          </Link>
        </Button>
...
export default function DashboardForm() {
  ...
  return (
    <div className="px-8 py-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">Actifs suivis</h1>
        {!loading && !error && (
