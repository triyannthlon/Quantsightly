...
/**
 * Colonnes de la watchlist CRYPTO (pipeline MARKETS).
 * ================================================
 *
 * Layout :
...
function HighDistances({
  ...
  if (state === "dash" || dist52w === undefined)
    return <span className="text-xs text-muted-foreground">—</span>;
  return (
...
const COL_TICKER: CryptoColumnDef = {
  key: "ticker",
  label: "Nom",
  align: "left",
...
const COL_CURRENCY: CryptoColumnDef = {
  key: "currency",
  label: "Devise",
  align: "left",
  width: "w-14",
...
const COL_LAST: CryptoColumnDef = {
  key: "last",
  label: "Dernier Prix",
  align: "right",
...
const COL_SPARKLINE: CryptoColumnDef = {
  key: "spk6m",
  label: "6 Mois",
  align: "center",
  width: "w-28",
...
const COL_DIST_52W: CryptoColumnDef = {
  key: "d52w",
  label: "Δ Sommet 52S / Δ ATH",
  align: "right",
  sortValue: (_, row) => row?.distanceTo52WHigh,
  hideSm: true,
  cell: (ctx) => (
    <HighDistances
      state={priceState(ctx)}
      dist52w={ctx.row?.distanceTo52WHigh}
      distATH={ctx.row?.distanceToATH}
    />
  ),
};
