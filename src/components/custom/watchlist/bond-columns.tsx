...
/**
 * Colonnes de la watchlist BOND (obligations souveraines, pipeline MARKETS).
 * ==========================================================================
 *
 * Layout :
 *   Ticker | Pays | Devise | Last | 6M | 1J | YTD | Δ sommet 52S
 *
 * Pas de logo (les bonds n'ont pas de logo société) — on utilise directement
 * `AssetTypeIcon` (icône Landmark) dans le TickerCell.
 *
 * Sémantique des couleurs : identique aux stocks.
 *  • Returns 1D/YTD            → vert >0, rouge <0, neutre =0
 *  • 52W High % (≤ 0 toujours) → NEUTRE (texte muted)
 */
...
/** Alias locaux : contexte + colonne paramétrés sur `BondWatchlistRow`. */
export type BondCellCtx = WatchlistCellCtx<BondWatchlistRow>;
export type BondColumnDef = WatchlistColumnDef<BondWatchlistRow>;

// ── États de cellule ─────────────────────────────────────────

type CellState = "skeleton" | "value" | "dash";

function priceState(ctx: BondCellCtx): CellState {
    ...
    <div className="flex items-center gap-2.5">
      ...
      <div className="min-w-0">
        <Tooltip>
          ...
          <TooltipContent side="top">{item.name ?? item.symbol}</TooltipContent>
        </Tooltip>
...
function CountryCell({ item }: BondCellCtx) {
...
const COL_TICKER: BondColumnDef = {
  key: "ticker",
  label: "Nom",
  align: "left",
...
const COL_COUNTRY: BondColumnDef = {
  key: "country",
  label: "Pays\n(Émetteur)",
  align: "center",
  cell: (ctx) => <CountryCell {...ctx} />,
};
const COL_CURRENCY: BondColumnDef = {
  key: "currency",
  label: "Devise",
  align: "center",
...
const COL_SPARKLINE: BondColumnDef = {
  key: "spk6m",
  label: "6 Mois",
  align: "center",
  width: "w-28",
...
const COL_DIST_52W: BondColumnDef = {
  key: "d52w",
  label: "Δ sommet 52S",
  align: "right",
...
export const BOND_COLUMNS: BondColumnDef[] = [
  COL_TICKER,
  COL_COUNTRY,
  COL_CURRENCY,
  COL_LAST,
