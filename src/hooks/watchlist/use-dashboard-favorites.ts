...
} from "@/lib/yann/watchlist/clients/watchlist-client";
...
async function fetchLayout(): Promise<string[]> {
    const res = await fetch("/api/me/dashboard/layout", { cache: "no-store" });
    if (!res.ok) return [];
...
export function useDashboardFavorites() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (ignore) return;
                setState({ favorites: [], loading: false, error: err instanceof Error ? err.message : "Erreur de chargement" });
            }
        })();
    ...
    const removeFavorite = useCallback(async (item: DashboardFavorite) => {
        ...
        try {
            ...
            toast.info("Retiré du dashboard principal", { description: item.name ?? item.symbol, duration: 4000 });
        } catch {
            setRefresh(n => n + 1);
        }
    }, []);
