...
import { buildCryptoRow } from "@/lib/yann/watchlist/builder/asset-crypto";
...
export function useAssetCryptoWatchlist(items: EnrichedWatchlistItem[]) {
    return useWatchlistRows<CryptoWatchlistRow>(
        items,
        (item, raw) => buildCryptoRow({
            identity   : {
                             symbol   : item.symbol,
