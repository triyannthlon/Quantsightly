...
import { buildForexRow } from "@/lib/markets/watchlist/builder/asset-forex";
...
export function useAssetForexWatchlist(items: EnrichedWatchlistItem[]) {
  return useWatchlistRows<ForexWatchlistRow>(items, (item, raw) =>
    buildForexRow({
      identity: {
        symbol: item.symbol,
