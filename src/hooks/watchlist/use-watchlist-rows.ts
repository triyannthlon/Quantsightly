...
import { fetchHistory, type HistoryResult } from "@/lib/yann/clients/history";
...
export function useWatchlistRows<TRow>(
    ...
    useEffect(() => {
        ...
        if (list.length === 0) { setRefreshing(false); return; }
        ...
        const handle = async (item: EnrichedWatchlistItem, forceThis: boolean): Promise<void> => {
            ...
            if (raw.status === "loading") {
                ...
                if (attempts[item.symbol] < MAX_ATTEMPTS) {
                    timers.push(setTimeout(() => { void handle(item, false); }, POLL_MS));
                } else {
