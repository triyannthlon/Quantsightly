/**
 * Couche MARKETS — types propriétaires de watchlist
 * ==============================================
 *
 ...
 *    brute ('@/lib/markets/analytics/metrics') lorsqu'ils servent :
 *    `sector', `marketCap', `pe` de `StockFundamentals', ...
 *  - On AJOUTE les champs supplémentaires qui seront calculés par MARKETS
...
export type Distance52WHigh = {
  /** Distance (en %) du close courant au PLUS HAUT close des 252 dernières
                                  ...
                                  Voir `distanceTo52WHigh()` dans `markets/metrics`. */
 ...
 */
export type StockWatchlistRow = { kind: "stock" } & WatchlistIdentity &
  CountryLocation & {
 ...
 * Colonnes : Name | Pays | Last | 1D | 1W | 1M | YTD | 52W High
 */
export type EtfWatchlistRow = { kind: "etf" } & WatchlistIdentity &
  CountryLocation & {
    logoUrl?: string;
  } & LastPrice &
  ReturnsWeekdayMultiHorizon &
...
export type CryptoWatchlistRow = { kind: "crypto" } & WatchlistIdentity &
  ...
  Distance52WHigh & { distanceToATH?: number } & Sparkline6M;
 ...
 */
export type IndexWatchlistRow = { kind: "index" } & WatchlistIdentity &
  CountryLocation & {
...
/**
 * Ligne de watchlist BOND (obligations souveraines)
 * Symbol format : "{CODE}.GBOND" — ex. "US10Y.GBOND", "FR10Y.GBOND".
 * Colonnes : Name | Pays | Devise | Last | 1D | 1W | 1M | YTD | 52W High
 * Pas de logo, pas de fundamentals, pas de dividends.
 */
export type BondWatchlistRow = { kind: "bond" } & WatchlistIdentity &
  CountryLocation &
  LastPrice &
  ReturnsWeekdayMultiHorizon &
  Distance52WHigh &
