...
export type AssetType = "stock" | "etf" | "crypto" | "currency" | "index" | "bond";
...
export async function getWatchlist(type: AssetType): Promise<WatchlistResponse> {
  const res = await fetchAuth(`/api/me/watchlist/${type}`, { cache: "no-store" });
...
export async function addToWatchlist(
  ...
  const res = await fetchAuth(`/api/me/watchlist/${type}/items`, {
    method: "POST",
...
export async function removeFromWatchlist(type: AssetType, itemId: string): Promise<void> {
  const res = await fetchAuth(`/api/me/watchlist/${type}/items/${itemId}`, { method: "DELETE" });
...
export async function setFavorite(type: AssetType, itemId: string, value: boolean): Promise<void> {
  const res = await fetchAuth(`/api/me/watchlist/${type}/items/${itemId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
