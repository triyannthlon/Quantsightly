/**
 * Couche MARKETS — normalisation d'une série brute
 * =============================================
 *
...
export function buildNormalizedSeries(rawBars: EodBar[], kind: SeriesKind): NormalizedSeries {
  ...
  const sorted = [...rawBars]
    .filter((b): b is EodBar & { close: number } => typeof b.close === "number")
    .sort((a, b) =>
      a.date.localeCompare(b.date),
    ); /*   - on rejette les barres sans close exploitable - on trie par date croissante (string compare = date compare pour YYYY-MM-DD) */
  if (sorted.length === 0) {
    return { kind, bars: [], source: { from: "", to: "" } };
  } /* Cas dégénéré : aucune cotation valide → série vide */
  ...
  for (const targetDate of calendar) {
    ...
    if (idx < 0)
      continue; /* ne devrait pas arriver : le calendrier commence à `from` qui est dans `sorted` */

    const src = sorted[idx];
    const isReal = src.date === targetDate;
    const adj = src.adjusted_close ?? src.close;

    bars.push({ date: targetDate, close: src.close, adjusted_close: adj, synthetic: !isReal });
  } //(1)
