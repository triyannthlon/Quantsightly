/**
 * Couche YANN — métriques sur série normalisée
 * ============================================
 *
...
export function formatPrice(value: number | undefined, currency?: string): string {
    ...
    if (currency && /^[A-Z]{3}$/.test(currency)) {
        ...
        } catch { /* devise non reconnue par Intl */ }
    }
...
function findBarIndex(bars: NormalizedBar[], targetDate: string): number
         ...
         while (lo <= hi)//(1)
               {//(1)
                  const mid = (lo + hi) >>> 1;
               if (bars[mid].date <= targetDate) {result = mid;lo = mid + 1;}
               else                              {             hi = mid - 1;}
               }//(1)
...
/************** weekdayReturns *****/
export function weekdayReturns(series: NormalizedSeries): { ret1d  ?: number;
                                                            ret1w  ?: number;
                                                            ret1m  ?: number;
                                                            retYtd ?: number; }
               {//weekdayReturns
               ...
               return { ret1d : pct(lastAdj, at(isoDaysBefore  (anchor, 1))),
...
export function distanceTo52WHigh(series: NormalizedSeries): number | undefined
       ...
       for (let i = start; i < bars.length; i++)//(1)
           {//(1)
           if (bars[i].close > maxClose) {maxClose = bars[i].close;}
           }//(1)
...
export function computePanelMetrics(series: NormalizedSeries): PanelMetrics
       ...
       let maxDD     = 0;
           for (const b of bars)//(b)
               {//(b)
               const p = b.adjusted_close;
                   if (p > peakAll) peakAll = p;
                   if (peakAll > 0) { const dd = (p / peakAll - 1) * 100; if (dd < maxDD) maxDD = dd; }
               }//(b)
