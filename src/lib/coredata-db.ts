...
export const codedataPool =
  globalForCodedata.codedataPool ??
  new Pool({ connectionString: process.env.CODEDATA_DATABASE_URL });

if (process.env.NODE_ENV !== "production") {
  globalForCodedata.codedataPool = codedataPool;
}
...
export async function findSeries(filter: SeriesFilter): Promise<SeriesMatch[]> {
  ...
  let ccyClause = "";
  if (filter.ccy) {
    params.push(filter.ccy);
    ccyClause = `AND ccy = $${params.length}`;
  }
...
export async function getDistinctValues(field: DistinctField): Promise<string[]> {
  ...
  const { rows } = await codedataPool.query<{ value: string }>(
    `SELECT DISTINCT ${col} AS value FROM economic_series ORDER BY ${col}`,
  );
